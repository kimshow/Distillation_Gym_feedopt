...
class CONFIG:
    ...
    def get_config(self):
        ...
        if self.config_number == 0:  # ChemSepExample LuybenPart
        ...
        elif self.config_number == 5:  # loosely based on Luyben hydrocarbon example
            compound_names = ["Isobutane", "N-butane", "Iso-pentane", "N-pentane"]
            Heating_value = np.array([49.5, 49.4, 55.2, 55.2])  # MJ/kg
            Price_per_MBTU = np.array([5.79, 5.31, 10.41, 10.41])  # $/Million Btu
            Molar_weights = np.array([58.124, 58.124, 72.151, 72.151])  # g/mol
            print("Haven't yet checked Iso-butane assuming same as pentane here")
            MJ_per_MBTU = 1055.06
            sales_prices = Price_per_MBTU / MJ_per_MBTU * Heating_value * (Molar_weights / 1000)  # now in $/mol
            COCO_file = os.path.join(os.getcwd(), "Env\PPDPexample810.fsd")
            fail_solve_penalty = 0.5
            required_purity = [0.99, 0.99, 0.99, 0.99]
            standard_args = COCO_file, sales_prices, fail_solve_penalty, required_purity
            return standard_args

        elif self.config_number == 6:  # loosely based on Luyben hydrocarbon example
            compound_names = ["Methane", "Ethane", "Propane", "Isobutane", "N-butane", "Iso-pentane", "N-pentane", "N-hexane", "n-heptane"]
            Heating_value = np.array([55.6, 51.9, 50.4, 49.5, 49.4, 55.2, 55.2, 47.7, 48.5])  # MJ/kg
            Price_per_MBTU = np.array([2.83, 2.54, 4.27, 5.79, 5.31, 10.41, 10.41, 10.41, 10.41])  # $/Million Btu
            Molar_weights = np.array([16.043, 30.07, 44.097, 58.124, 58.124, 72.151, 72.151, 86.18, 100.21])  # g/mol
            print("Haven't yet checked Iso-butane, N-hexane and n-heptane prices, assuming same as pentane here")
            ...
            sales_prices = Price_per_MBTU / MJ_per_MBTU * Heating_value * (Molar_weights / 1000)  # now in $/mol
            sales_prices = np.concatenate((sales_prices, [0]))  # nitrogen
            COCO_file = os.path.join(os.getcwd(), "Env\casestudy1_NGL.fsd")
            fail_solve_penalty = 3
            required_purity = [0, 0.95, 0.90, 0,99, 0.95, 0, 0, 0, 0]
            standard_args = COCO_file, sales_prices, fail_solve_penalty, required_purity
            return standard_args
